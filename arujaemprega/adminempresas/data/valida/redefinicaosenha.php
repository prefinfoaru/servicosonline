		...
		$resultado_consulta = mysqli_query($conn, $consulta);
	 	...
	 	if($resultado_consulta){
			...
			if($email === $row_dados['email']){
					$id_solicitante =  $row_dados['id_cadastroempresa'];
					$nome = utf8_encode($row_dados['razao_social']);
					$data =  date("Y-m-d");
					$datam = date("m");
					$ale = rand();
					$protocolo =  $id_solicitante.$ale.$datam;
				$con = "SELECT * FROM bd_emprega_aruja.tb_redefinitsenha_empresa where id_solicitante = '$id_solicitante' and data_solicitacao = '$data';";
				...
				if(mysqli_fetch_assoc($resultado_con) >= '1'){
				...
				else{// se ele não fez solitação no dia de hoje
					...
					<a href="https://servicosonline.prefeituradearuja.sp.gov.br/servicosonline/arujaemprega/adminempresas/pages/linkredefinirsenha.php?id='.$protocolo.'">' ;
				   ...
				   // //Corpo da mensagem em texto
					...
					if($mail->Send()){
						...
						$insert = "INSERT INTO `bd_emprega_aruja`.`tb_redefinitsenha_empresa` (`id_solicitante`, `protocolo`, `data_solicitacao`, `status`) VALUES ('$id_solicitante', '$protocolo', '$data', '0');";
			...
			}else{// email não cadastrado no banco
			...
			echo  "<script>alert('Cpf/Cnpj não cadastrado!')</script>";
			echo 	"<script>window.location.href ='../../pages/login.php'</script>"; 
			exit();
