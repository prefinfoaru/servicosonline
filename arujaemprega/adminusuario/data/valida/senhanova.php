...
$prot 		 = filter_input (INPUT_POST, 'prot'	, FILTER_SANITIZE_STRING); 
...
if(!$response->success){//captcha errado
	...
	$consulta = "SELECT * FROM bd_emprega_aruja.tb_redefinirsenha_usuario where protocolo = '$prot';";
	...
	if($row_dados >= '1'){
		...
		if($dia >= '1' || $status == '1'){
			...
			$Atualiza = "UPDATE `bd_emprega_aruja`.`tb_redefinirsenha_usuario` SET `status`='1' WHERE `protocolo`='$prot'";
			...
			if($resultado_atualiza){
				...
				$insert = "UPDATE `bd_emprega_aruja`.`tb_cadastro_solicitante` SET `senha_acesso`='$hash' WHERE `id_solicitante` = '$id';";
