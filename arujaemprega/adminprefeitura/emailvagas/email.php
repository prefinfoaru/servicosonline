...
foreach ($pdo->query($queryEmails) as $rowEmail)
{
    ...
    // Verifica se o solicitante tem área desejada cadastrada
    $queryArea  = $pdo->query("SELECT area_desejada FROM bd_emprega_aruja.tb_objetivo_profissional WHERE id_solicitante = $id ORDER BY id_obj_prof DESC LIMIT 1");
    $rowArea    = $queryArea->fetch();

    if ($rowArea['area_desejada'])
    {
        /* SELECIONA VAGA DE ACORDO COM A ÁREA DESEJADA */
        $areaDesejada = $rowArea['area_desejada'];
        $queryVagaArea = 
        "SELECT 
            cv.id_vaga,
            cv.titulo,
        ...
        WHERE
            profissao = '$areaDesejada'
        ORDER BY cv.data DESC LIMIT 3";

        $vagas = array();

        foreach ($pdo->query($queryVagaArea) as $rowAreaDesejada)
        {
            $vagaAreaDesejada = '
            ...
            <div class="row">
                <div>
                    <p>
                        <a href="https://servicosonline.prefeituradearuja.sp.gov.br/servicosonline/arujaemprega/adminusuario/pages/loginvisu_vaga.php?vaga='.$rowAreaDesejada['id_vaga'].'&?ret=home" target="blank"><strong style="color: #3498db;">'.strtoupper($rowAreaDesejada['titulo']).'</strong></a> <span class="text-muted"> - Salário R$: '.$rowAreaDesejada['salario'].'</span> <br>
                        <p class="text-muted">'.substr(strip_tags($rowAreaDesejada['descricao']), 0, 97).'...</p>
                    </p>
                </div>
            </div>
            ...
            array_push($vagas, $vagaAreaDesejada);
        }
    }
    else
    {
        ...
        // echo $hierarquia_minima;
        // echo $hierarquia_maxima;
        ...
        foreach ($pdo->query($queryVagas) as $rowNivel)
        {
            $vaga = '<div class="row">
                        <div>
                            <p>
                                <a href="https://servicosonline.prefeituradearuja.sp.gov.br/servicosonline/arujaemprega/adminusuario/pages/loginvisu_vaga.php?vaga='.$rowNivel['id_vaga'].'&?ret=home" target="blank"><strong style="color: #3498db;">'.strtoupper($rowNivel['titulo']).'</strong></a> <span class="text-muted"> - Salário R$: '.$rowNivel['salario'].'</span> <br>
                                <p class="text-muted">'.substr(strip_tags($rowNivel['descricao']), 0, 97).'...</p>
                            </p>
                        </div>
                    </div>';
    ...
    $mail->Subject    = utf8_decode("Arujá Emprega");
    $mail->From       = "noreply@aruja.sp.gov.br";
    $mail->FromName   = utf8_decode("Prefeitura do Município de Arujá");	
