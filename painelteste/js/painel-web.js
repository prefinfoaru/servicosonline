        ...
        .controller('PainelCtrl', function ($scope, $http) {
            ...
            $scope.chamar = function () {
                if (PainelWeb.started && $scope.senhas.length > 0) {
                    ...
                    var convencional = new SpeechSynthesisUtterance('Hello World');
                    PainelWeb.trigger('callstart');

                    // som e animacao
                    PainelWeb.Alert.play($scope.config.alert, !$scope.config.vocalizar);
                    if ($scope.config.vocalizar) {
                        speechSynthesis.speak(new SpeechSynthesisUtterance(senha.nomeCliente));
                        speechSynthesis.speak(new SpeechSynthesisUtterance("Senha"+senha.texto+senha.local+senha.numeroLocal));
                        // setTimeout(function(){
                            // speechSynthesis.speak(new SpeechSynthesisUtterance("Senha"+senha.texto+senha.local+senha.numeroLocal))
                    ...
                    $scope.ultima = senha;
                    $("#senhaAtual").html('<center><h1>Senha</h1><font size="15pt">' + senha.texto + '</font></center>');

                    $('#modalSenhaAtual').modal('show');
                    setTimeout(function () {
                        $('#modalSenhaAtual').modal('hide');
                    }, 3000);
