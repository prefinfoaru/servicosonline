...
$result_usuario = "SELECT id_cad_user,matricula, nome, email, senha FROM bd_sspf.tb_cadastro_de_usuarios WHERE matricula='$usuario' LIMIT 1";
$resultado_usuario = mysqli_query($conn, $result_usuario);
$row_usuario = mysqli_fetch_assoc($resultado_usuario);

$row_usuario['matricula'] ;

if($row_usuario['matricula'] != $usuario) {
	
	
	$_SESSION['msg'] = " * Matricula não encontrada.";
...
require 'PHPMailer/PHPMailerAutoload.php';
			...
			$update_cad = $update_cad = "UPDATE `bd_sol_hr`.`tb_cadastro_de_usuarios` SET `senha`='$hash', `data_cadastro`='$data2' WHERE `matricula`='$row_usuario[matricula]';";
