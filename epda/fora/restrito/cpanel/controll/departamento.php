...
<div class="container">
    ...
    <div align="center"><br>
        ...
        <label style="font-family: consolas">Relatório por Departamento <?php echo $data ?></label></div>
    <br>
    <div class="container">
        <?php
        ...
        if ($filtro == "todos") {
            $sql = $pdo->query("SELECT * FROM tb_solicitacao where tb_situacao <>1 AND tb_departamento_funcionario = '$departamento' AND tb_data_status between '$datai' AND '$dataf' ORDER BY tb_nome_funcionario ASC");
        } else {
            $sql = $pdo->query("SELECT * FROM tb_solicitacao where tb_situacao ='$filtro' AND tb_departamento_funcionario = '$departamento' AND tb_data_status between '$datai' AND '$dataf' ORDER BY tb_nome_funcionario ASC");
        }

        $sqltotal = $pdo->query("SELECT tb_departamento_funcionario, COUNT(*) As Total
                FROM tb_solicitacao
                where tb_situacao <>1 AND tb_departamento_funcionario = '$departamento' AND tb_data_status between '$datai' AND '$dataf'");
        $total = $sqltotal->fetchAll();
        $sqlapv = $pdo->query("SELECT tb_departamento_funcionario, COUNT(*) As Total
                FROM tb_solicitacao
                WHERE tb_situacao = 2 AND tb_departamento_funcionario = '$departamento' AND tb_data_status between '$datai' AND '$dataf'");
        $apv = $sqlapv->fetchAll();
        $sqlneg = $pdo->query(" SELECT tb_departamento_funcionario, COUNT(*) As Total
                FROM tb_solicitacao
                WHERE tb_situacao = 3 AND tb_departamento_funcionario = '$departamento' AND tb_data_status between '$datai' AND '$dataf'");
        $neg = $sqlneg->fetchAll();
        ?>
        <div align="center"><br>
        ...
        <table class="table table-bordered">
            <thead>
                <tr style="background:#D3D2D2; text-align: center">
                    <th style="text-align: center">Total de Solicitações do Departamento</th>
            ...
            <tbody>
                <tr>
                    ...
                    if ($sql->rowCount() >= 1) {
                        ...
                        <div align="center"><br>
                            <label style="font-family: consolas">Não há informação para este departamento neste periodo</label></div>
                        <br>
                    <?php
                    }
                    ?>
            ...
            <tbody>
                <tr>
                    ...
                    foreach ($sql as $row) {
                        ...
                        echo '<td title="' . $row['tb_departamento_funcionario'] . '">' . $row['tb_departamento_funcionario'] . '</td>';
